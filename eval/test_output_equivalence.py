...
def run_output_equivalence_test(
    ...
    from dfloat11_tt.nn.hf_patch import (
        clear_df11_weight_caches,
        clear_tt_reference_weight_caches,
    ...
    logger.info(
        ...
        f"compare_logits={compare_logits}, reference_backend={reference_backend}"
    )
    ...
    if reference_backend == "ttnn" and tt_device is not None:
        logger.info("[output_equiv] Loading uncompressed TTNN reference model ...")
        ref_model = from_pretrained_tt_reference(model_id, tt_device, patterns=patterns)
    else:
        if reference_backend == "ttnn" and tt_device is None:
            logger.warning(
                "[output_equiv] reference_backend=ttnn requested without TT device; using CPU reference."
            )
            reference_backend = "cpu"
        logger.info("[output_equiv] Loading BF16 CPU reference model ...")
    ...
    for i, prompt in enumerate(prompts):
        ...
        with torch.no_grad():
            logger.info(f"[output_equiv] [{i+1}/{len(prompts)}] reference generate start")
            t0 = time.perf_counter()
            ref_out = ref_model.generate(**_generation_kwargs(ref_model, inputs))
            ref_seconds = time.perf_counter() - t0
            logger.info(
                f"[output_equiv] [{i+1}/{len(prompts)}] reference generate done "
        ...
        first_ref_token = None
        first_df11_token = None
        for token_index, (ref_token, df11_token) in enumerate(zip(ref_tokens, df11_tokens)):
            if ref_token != df11_token:
                first_mismatch_index = token_index
                first_ref_token = ref_token
                first_df11_token = df11_token
                break
        if first_mismatch_index is None and len(ref_tokens) != len(df11_tokens):
            first_mismatch_index = min(len(ref_tokens), len(df11_tokens))

        # Compare first generated-step logit tensor bit-exactly when requested.
        logits_match = None
        if compare_logits and tt_device is not None:
            if ref_out.logits and df11_out.logits:
        ...
        passed = tokens_match and (logits_match is not False)
        if not passed:
            all_pass = False
        ...
        results.append({
            ...
            "first_ref_token": first_ref_token,
            "first_df11_token": first_df11_token,
        ...
        if first_mismatch_index is not None:
            logger.info(
                f"    first token mismatch at index {first_mismatch_index}: "
                f"ref={first_ref_token}, df11={first_df11_token}"
            )
        logger.info(f"    reference completion: {ref_completion!r}")
        logger.info(f"    DF11 completion: {df11_completion!r}")
        logger.info(
            f"    timing: ref={ref_seconds:.2f}s, DF11={df11_seconds:.2f}s, "
            f"DF11 tok/s={df11_tokens_per_sec or 0:.2f}"
        )

    if reference_backend == "ttnn":
        clear_tt_reference_weight_caches(ref_model)
    if tt_device is not None:
